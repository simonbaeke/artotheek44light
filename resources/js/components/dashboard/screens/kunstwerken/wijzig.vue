<template>
    <v-card flat v-if="!artworkLoading">
        <v-card-text>
            <v-form>
                <v-container>
                    <v-layout row wrap>
        ...
        <v-card-actions>
            <v-btn outlined @click="save" :loading="artworkSaving">Opslaan</v-btn>
            <v-btn outlined>Annuleer</v-btn>
        </v-card-actions>
    </v-card>
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                this.update({id: this.artwork.id, data: formData}).then((response) => {
        ...
        mounted() {
            ...
            //check of kunstwerk in lijst staat

            new Promise((resolve, reject) => {
                if (this.kunstwerk == null) {
                    this.getKunstwerk(this.$route.params.id).then(() => resolve())
                }
                else {
                    resolve()
                }
                ...
                this.type_id = this.kunstwerk.type.id
                this.artist_id = this.kunstwerk.artist.id

                this.artwork = new Form({
                    id:this.kunstwerk.id,
                    name: this.kunstwerk.name,
                    type_id: this.kunstwerk.type.id,
                    artist_id: this.kunstwerk.artist.id,
                    thumbnailurl: "",
                    price: this.kunstwerk.price,
                    length: this.kunstwerk.length,
                    width: this.kunstwerk.width,
                    height: this.kunstwerk.height,
                    code: this.kunstwerk.code,
                    forsale: this.kunstwerk.forsale
                })
